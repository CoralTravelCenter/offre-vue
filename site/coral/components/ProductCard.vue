...
<template>
	<div ref="$el" class="product-card">
		<div class="visual-details">
			<a target="_blank" :href="offerHref" class="visual"
				 :style="{ backgroundImage: `url(${ hotel.images[0].sizes.find(s => s.type===4).url })` }">
			...
			<div class="details">
				<div class="location" :class="{ 'has-coordinates': !!hotel.coordinates }"
						 @click="handleHotelLocationClick(hotel)">{{ hotel.locationSummary }}
				</div>
		...
		<div class="pricing">
			<div class="tour-type">
				<button v-if="!isHotelOnly" class="package" :class="{ selected: tourType === 'package' }"
								@click="tourType = 'package'">Тур с перелетом
				</button>
				<button class="only-hotel" :class="{ selected: isHotelOnly || tourType === 'hotel' }"
								@click="tourType = 'hotel'">Только отель
				</button>
			</div>
