...
<template>
	...
	>
		<RegionSelect
			class="offre-controls__region controls__region col-[1/-1] row-1 min-w-0 -mx-2 min-[1024px]:col-1 min-[1024px]:row-1 min-[1024px]:mx-0"
		...
		<div v-if="regionsLoading" class="offre-controls__slot-skeleton controls__departure col-1 row-2 min-w-0 min-[1024px]:col-2 min-[1024px]:row-1">
			<Skeleton class="offre-controls__skeleton offre-controls__skeleton--select h-10 w-full rounded-[8px]"/>
		</div>
		<Select v-else v-model="selectedDepartureIdModel">
			...
			>
				...
				<SelectValue placeholder="Город вылета" class="offre-controls__trigger-value min-w-0 max-w-full grow basis-0 truncate"/>
			</SelectTrigger>
			<SelectContent>
				<SelectItem
		...
		<div v-if="regionsLoading && isTimeframeSelectable" class="offre-controls__slot-skeleton controls__timeframe col-2 row-2 min-w-0 min-[1024px]:col-3 min-[1024px]:row-1">
			<Skeleton class="offre-controls__skeleton offre-controls__skeleton--select h-10 w-full rounded-[8px]"/>
		</div>
		<Select v-else-if="isTimeframeSelectable" v-model="selectedTimeframeModel">
			...
			>
				<svg
					v-show="!breakpointKey"
					class="offre-controls__icon offre-controls__icon--calendar inline-flex h-[18px] w-[18px] shrink-0 basis-[18px] text-foreground/70"
					viewBox="0 0 18 16"
					fill="none"
					aria-hidden="true"
				>
		...
		>
			<Skeleton class="offre-controls__skeleton offre-controls__skeleton--toggle h-10 w-10 rounded-[8px]"/>
		</div>
		<ViewModeToggle
			v-else
