...
<template>
	<div ref="$el" class="product-grid flex flex-col text-[16px]" :data-instance-uuid="instance_uuid">
		...
		<div v-if="viewMode === 'map'" class="map-view relative aspect-video min-h-110 overflow-hidden rounded-xl">
			<yandex-map
					v-model="map"
				...
				<yandex-map-clusterer v-model="clusterer"
					...
					<template #cluster="{ length, clusterer }">
						<div
							...
							<div
									class="hud absolute inset-0 grid place-content-center rounded-full border-2 border-white bg-primary text-primary-foreground shadow-[0_1px_2px_rgba(0,0,0,0.2)]">
								{{ length }}
							</div>
							<template v-for="(range, idx) in [getClusterPriceRange(clusterer.features)]" :key="idx">
								<div
										class="pricing absolute -z-1 top-0.5 flex h-[2.8em] flex-col items-end justify-evenly overflow-hidden rounded-[100px_3em_3em_100px] bg-white/90 px-[0.5em] pl-[3.5em] text-[0.75em] indent-[-10em] opacity-0 invisible shadow-none text-nowrap backdrop-blur-xs transition-[text-indent,opacity,box-shadow,visibility] duration-250 group-hover:indent-0 group-hover:opacity-100 group-hover:visible group-hover:shadow-[0_1px_2px_rgba(0,0,0,0.2)]">
					...
					<div class="relative">
						...
						/>
							<ProductMapOverlayCard
									class="pointer-events-auto hidden min-[1024px]:absolute min-[1024px]:left-2 min-[1024px]:bottom-14 min-[1024px]:z-20 min-[1024px]:block min-[1024px]:w-90 min-[1024px]:max-w-[92vw] min-[1024px]:mb-0"
									:product="product"
			...
			>
				<ProductMapOverlayCard class="pointer-events-auto" :product="activeMapProduct" @close="handleOverlayClose"/>
			</div>
		</div>
